...
let lrs_accumulation = [];
let nb_accumulation = 0;
export async function prepareLrsComponent(indexProduct, index) {
...
async function getLrsData(indexProduct, index, page) {
    try {
 ...
 * update component
 */
async function updateLrsComponent(indexProduct, index, data) {
    const config = await getConfig();
    const currentLrs = config.products[indexProduct].lrs[index];
...
function updateLrsComponentTable(indexProduct, index, data, more, currentLrs) {
    const result = currentLrs.result;
    const table_id = result.table.dom_id;
    const arrayVerbs = currentLrs.verbs.split(",");
    nb_accumulation += 1;
