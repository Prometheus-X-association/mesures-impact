...
export async function prepareSurveyComponent(indexProduct, index) {
...
async function getSurveyData(indexProduct, index) {
    try {
        const config = await getConfig();
 ...
 * update component
 */
async function updateSurveyComponentTable(indexProduct, index, data) {
    const config = await getConfig();
    const currentSurvey = config.products[indexProduct].surveys[index];
